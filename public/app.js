...
async function searchLusha() {
  ...
  try {
    ...
    const sizeMap = { '1,10':[{min:1,max:10}],'11,50':[{min:11,max:50}],'51,200':[{min:51,max:200}],'201,500':[{min:201,max:500}],'501,1000':[{min:501,max:1000}],'1001,':[{min:1001,max:100000}] };
    const sizes = sizeMap[sizeRaw] || [];
    const deptMap = { 'CEO':'C-Suite','Founder':'C-Suite','Co-Founder':'C-Suite','Managing Director':'C-Suite','CMO':'Marketing','VP Marketing':'Marketing','Head of Marketing':'Marketing','Director of Marketing':'Marketing','Head of Sales':'Sales','VP Sales':'Sales','Director of Sales':'Sales','CTO':'Engineering & Technical','CFO':'Finance','HR Director':'Human Resources' };
    const departments = titles.length ? [...new Set(titles.map(t => deptMap[t] || 'C-Suite'))] : [];
    const industryMap = { 'Legal Services':7,'Accounting':1,'Financial Services':6,'Management Consulting':10,'Computer Software':3,'SaaS':3,'Real Estate':14,'E-Learning':5,'Human Resources':8,'Marketing':11,'Healthcare':9,'Education':5 };
    const industryIds = industries.length ? [...new Set(industries.map(i => industryMap[i]).filter(Boolean))] : [];
    const contactInclude = {};
    const companyInclude = {};
    if (departments.length)    contactInclude.departments         = departments;
    if (locationFilter.length) contactInclude.locations           = locationFilter;
    if (industryIds.length)    companyInclude.mainIndustriesIds   = industryIds;
    ...
    const res  = await fetch(`${LUSHA_PROXY}/search`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(lushaBody) });
    if (!res.ok) { const err = await res.json().catch(() => ({})); throw new Error(err.error || `HTTP ${res.status}`); }
    const data   = await res.json();
    const people = data.data || [];
    if (!people.length) { showToast('No Lusha results — try broader filters', 'info'); return; }
    apolloResults = people.map(p => ({
      ...
      _lusha: true, _contact_id: p.contactId, _has_email: p.hasEmails, _has_phone: p.hasPhones,  _person_id:  p.personId,    // ← ADD THIS
    }));
